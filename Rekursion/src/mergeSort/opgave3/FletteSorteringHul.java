...
public class FletteSorteringHul {
	...
	private void merge(ArrayList<Integer> list, int low, int middle, int high) {
		ArrayList<Integer> temp = new ArrayList<Integer>();//sub array der holder værdierne
		int counter1 = low;
		int counter2 = middle+1;

		while (counter1 <= middle && counter2 <= high){//Dene del starter sortering
			if(list.get(counter1) < list.get(counter2)){ // er indhold for index counter 1 mindre end 2
...
//				}
		...
		while (counter1 <= middle){ // adder den sidste del for counter1 <= middle
			temp.add(list.get(counter1));
			counter1++;
		...
		while (counter2 <= high){ //adder sidte del for counter <= high
			temp.add(list.get(counter2));
			counter2++	;


		}
		for(int i = 0; i < temp.size(); i++){ // overføre fra temp til lsit
